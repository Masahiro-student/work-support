...
def toward_tool(start_time, count, r):
    ...
    while flag:
        ...
        pre_hand_center = (h_pred3[0],h_pred3[3])
        ...
        draw_prediction(color_arr,h_pred3)
        ...
        #guide(hand, object_center)
        guide_only_leftright(pre_hand_center, object_center)
        #wall(hand)
        #guide2(hand, target)
        #print(h_pred3)
...
def guide(hand, target):
    ...
    dx = target[0] - hand.center[0]
    dy = target[1] - hand.center[1]
    if abs(dx) > abs(dy):
        if dx < 0:
            mc.send_pwm(2, motor_power)
            mc.send_pwm(0, 0)
            mc.send_pwm(1, 0)
            print(2)
        else:
            mc.send_pwm(0, motor_power)
            mc.send_pwm(1, 0)
            mc.send_pwm(2, 0)
            print(0)
    else:
        if dy < 0:
            mc.send_pwm(1, motor_power)
            mc.send_pwm(0, 0)
            mc.send_pwm(2, 0)
            print(1)
        else:
...
def guide_only_leftright(pre_hand, target):
    dx = target[0] - pre_hand[0]
    #dy = target[1] - hand.center[1]
    if  dx < -object_width/2:
        mc.send_pwm(2, motor_power)
        mc.send_pwm(0, 0)
        mc.send_pwm(1, 0)
    elif dx > object_width/2:
        mc.send_pwm(0, motor_power)
        mc.send_pwm(1, 0)
        mc.send_pwm(2, 0)
    else:
