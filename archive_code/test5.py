...
def load_mask2():
    ...
    for i in range(279, 923):
        for j in range(170, 617):
            mask[j][i] = 1
    return mask
...
def toward_tool(start_time, count, r):
    ...
    print(marker2[1])'''
    cap0.set(cv2.CAP_PROP_SETTINGS, 1)
    ...
    while flag:
        ...
        #print(line_in_object(np_object2, center0, center1))
        #wall(hand)
        #guide2(hand, target)
        ...
        print(cap1.get(cv2.CAP_PROP_AUTO_EXPOSURE))
        print(cap0.get(cv2.CAP_PROP_BRIGHTNESS))
        print(cap1.get(cv2.CAP_PROP_BRIGHTNESS))
        print(cap0.get(cv2.CAP_PROP_CONTRAST))
