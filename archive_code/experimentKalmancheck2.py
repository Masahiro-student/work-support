...
goal_pt1 = (600, 450)
goal_pt2 = (730, 513)
goal_polygon = np.array([[600, 450], [600, 513], [730, 450], [730, 513]])
...
def toward_tool(start_time, count, r):
    ...
    while flag:
        ...
        draw_imaginary_line(color_arr, goal_pt1, goal_pt2)
        ...
        if deltax < 5:
            ...
            for i in [1,2]:
                x[i]  = 0
                xm1[i] = 0
            TF = dot_in_goal(goal_polygon, [hand.center[0],h_pred3[3]])
            print(TF)
            if TF == 1 or TF == 0:
                mc.drive_all(255)
            else:
            ...
            TF = dot_in_goal(goal_polygon, [h_pred3[0],h_pred3[3]])
            print(TF)
            if TF == 1 or TF == 0:
                mc.drive_all(255)
            else:
...
def dot_in_goal(polygon, pt):
    #return cv2.pointPolygonTest(polygon, pt, False) 
    x = int(pt[0])
    y = int(pt[1])
    if (600 <= x <= 730 and 450 <= y <= 513):
        return 1
    else:
        return -1
